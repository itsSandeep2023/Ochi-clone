...
function Navbar() {
  ...
  return (
    <div
      className={`w-full pt-[1vw] px-[3.5vw] min-h-10 font-['Neue_Montreal'] flex justify-between items-center fixed top-0 backdrop-blur-xs z-40`}
    >
      ...
      <div className="links ml-[15vw] flex gap-[2vw]">
        {links.map(({ label, href }, index) => (
          <a
            key={index}
            href={href}
            data-scroll-to
            className={`text-xs xl:text-xl lg:text-base md:text-sm ${
              index === 4 ? "ml-[15vw]" : ""
            } cursor-pointer hover:*:last:w-full *:last:duration-300`}
          >
            <div className="text-nowrap">{label}</div>
            <div className="line w-0 h-[1px] bg-zinc-900"></div>
          </a>
        ))}
      </div>
    </div>
